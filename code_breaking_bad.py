...
#Variable pour la couleur du texte
VERT = "\033[92m"
FOND_BLANC = "\033[47m"
NOIR = "\033[30m"
RESET = "\033[0m"
...
def fin_histoire():  #Affichage de la fin de l'histoire
	parole(" \n\n  --- 💸 FIN DE L'HISTOIRE 💸 ---  \n\n ", 0.05)
	print("""⢰⣶⣶⣶⣶⣶⣶⣶⣶⣶⣶⣶⣶⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⢸⣿⠛⠛⠛⠻⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⢸⣿⠀⣿⣿⠆⢸⣿⠿⢿⠿⢿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
...
def la_quête_du_crackhead(sachets, money):
	...
	réponse_crackhead = demander_choix("🔹 Que fais-tu ?", choix_possibles[0])
...
def distributeur_local(sachets, money):
	...
	réponse_distributeur = demander_choix("🔹 Que réponds-tu?", choix_possibles[0])
...
def mission_dealeur_1(money):
	sachets = 12
	#Dictionnaire contenant les mission pour dealer
	missions = {
	...
	while True:
		...
		if int(choix) == 1:
			...
			if purete < 90:
				...
				if blue_crystal < quantite :
					print(f"\n❌ Tu as déjà vendu une partie de ta METH lors de ta dernière transaction, il est impossible d'authentifier la pureté du reste de ton produit")
				if blue_crystal >= quantite :
...
def synthèse_blue_crystal(money, blue_crystal, purete, quantite):  #Menu pour cook de la METH
	purete = 0     
	quantite = 0 
...
def acheter(money, inv):   #Menu pour acheter
	while True:
		print("\n------ INVESTISSEMENTS ------")
		...
		if int(choix) == 1:
			...
			if money < 500000:
				...
				money -= 500000
				inv["avocat"] = True
				print("\nSaul Goodman rejoint ton organisation. « Better call Saul ! »")
				input("\n>>> Appuie pour continuer...")
		elif int(choix) == 4:
			if inv["informateur"]:
...
def mission_finale(money, inv, name):    #Menu mission finale
	...
	while True:
		...
		if int(choix) == 1:
			...
			confirm = demander_choix("\n🔹 Es-tu sûr de voiloir te retirer du deal ? (action iréversible) (1)oui/(2)non : ", choix_possibles[0])
			if int(confirm) == 1:
				...
				if inv["avocat"]:
					...
					time.sleep(1)
					choix = demander_choix("🔹 Accepter (1) ou Refuser (2) :", choix_possibles[0])
		...
		#Guerre contre les Salamenca
		elif int(choix) == 3:
			print("\nTu veux déclarer la guerre aux Salamanca…")
			print("C’est suicidaire sans préparation. ⚠️")
			...
			if inv["hommes de mains"] < 3 or inv["armes"] < 3:
				...
				confirm = demander_choix("\n🔹 Es-tu sûr de vouloir mener cette guerre ? (action iréversible) (1)oui/(2)non : ", choix_possibles[0])
				if int(confirm) == 1:
					...
					confirm = demander_choix("🔹 Que choisis-tu?", choix_possibles[0])
...
def menu_principal(money, lieu, blue_crystal, name, purete, quantite):  #Menu principale
	tot = blue_crystal
	vendu = 0
	# Dictionnaire avec l'inventaire des ressources dispo
	inv = {
		"armes": 0,
		"hommes de mains": 0,
		"avocat": False,
	...
	while True:
		...
		if int(menu) == 1:
			...
			money, blue_crystal, purete, quantite = vendre(money, blue_crystal, purete, quantite)     #Menu pour vendre
			input("\n>>>Appuie sur entrée pour retourner au menu...")
		elif int(menu) == 3:
			money, inv = acheter(money, inv)             #Menu achat
			input("\n>>>Appuie sur entrée pour retourner au menu...")
		elif int(menu) == 4:
			...
			confirm = demander_choix("\nEs-tu sûr de vouloir quitter le jeu ? (1)oui / (2)non : ", choix_possibles[0])
			if int(confirm) == 1:
				fin_histoire()
			elif int(confirm) == 2:
				time.sleep(0.5)
...
def intervention_GUS(money, prix, lieu, name):
	...
	if money < prix :
		...
		choix = demander_choix("🔹 Que choisis-tu ?", choix_possibles[0])
		if int(choix) == 1:
			...
			time.sleep(2)
			parole("Quelques heures plus tard, tu disparais mystérieusement.", 0.03)
			time.sleep(2)
	...
	if money > prix :
		...
		if int(choix) == 1:
			...
			choix_bis = demander_choix("🔹 Que fais-tu ?", choix_possibles[0])		
			if int(choix_bis) == 1:
...
def baron_de_la_drogue(money):
	...
	name = input("🔹 Quel nom veut tu porter à la place de Walter Black : ")   #Choix du nom
	...
	choix = demander_choix("🔹 Ton choix : ", choix_possibles[0])
	if int(choix) == 1:
		prix = 20000
		lieu = "camping-car"
	elif int(choix) == 2:
	...
	money, lieu, blue_crystal, name, purete, quantite = menu_principal(money, lieu, blue_crystal, name, purete, quantite)  #Appel de la fonction menu
...
def faire_tomber_TUCO(money):
	...
	choix = demander_choix("🔹 Comment veux-tu t’y prendre ? ", choix_possibles[1])
	if int(choix) == 1:
		...
		choix = demander_choix("🔹 Que fais-tu ? ", choix_possibles[0])
		if int(choix) == 1:
	...
	if int(choix) == 2:
		...
		if reussite == 3 :  # 2 chances sur 3 de réussir à tuer TUCO
	...
	if int(choix) == 3:
		...
		choix = demander_choix("🔹 Ton choix : ", choix_possibles[0])
		reussite = random.randint(1,3)  # 2 chances sur 3 de réussir à tuer TUCO
		print("\nTuco arrive...")
		if reussite == 3:
...
def travailler_pour_TUCO(money, trahison):
	...
	if int(choix) == 1:
		...
		programme = demander_choix("🔹 Quel paramètre t'intéresses ? ", choix_possibles[0])  
		if int(programme) == 1:
		...
		cuisson = demander_choix("🔹 Quel paramètre chois-tu ? : ", choix_possibles[0])
		if int(cuisson) == 1:
			...
			if random.randint(1, 2) == 1:   #1 chance sur 2 de tout faire péter
				parole("\n💥💥BOOOOM !!!!💥💥", 0.07)
				time.sleep(1)
...
def labo_de_TUCO(money):
	...
	time.sleep(1)
	trahison = False
	while not trahison:  #Tant que tu n'es pas trahit tu continue de bosser pour TUCO
	...
	money = faire_tomber_TUCO(money) #Trahison tu veux le faire tomber 
...
def le_cartel(money): 
	...
	time.sleep(1)
	_, money = mission_dealeur_1(money)     #Appel de la fonction pour vendre les sachets de drogues
	...
	if money < 10000:
		...
		money = fuite(money)  #Appel de la fonction fuite
	else:
		...
		time.sleep(2)
		money = labo_de_TUCO(money) #Appel de la fonction Labo 
...
def introduction(money):     #Introduction de l'histoire avant d'arriver dans le narcotrafique 
	...
	if int(reponse_1) == 1:
		...
		reponse_1_1 = demander_choix("🔹 Que choisissez-vous ? : ", choix_possibles[0])
		...
		if int(reponse_1_1) == 1: 
			...
			reponse_1_1_1 = demander_choix("🔹 Que choisissez-vous ? : ", choix_possibles[0])
		...
		# Branche Casino
			...
			time.sleep(1)
			tours = 0
			while tours < 5:  #On peut gagner max 5 fois
				print(f"\nTour {tours+1} — Mise actuelle : {money}$")
				print("Vous choisissez de mettre tout votre argent sur le :\n 1)Rouge🔴\n 2)Noir⚫️")
				choix_casino = demander_choix("🔹 Quel est votre choix?", choix_possibles[0])
				resultat = random.randint(1, 2)     # 1 chance sur 2
				time.sleep(1)
				parole("\nLa roue tourne ... ♣️♦️♠️♥️", 0.03)
				time.sleep(0.5)
				if int(choix_casino) == resultat:
					money = 2*money     # Argent doublé 
					tours = tours + 1
					parole(f"\nBravo ! Tu gagnes. Ton argent double : {money}$.", 0.01)
					if tours == 5:
						parole(f"JACKPOT ! Tu as gagné 5 fois d’affilée.", 0.01) 
						parole(f"Tu repars avec {money}$ !!! Vous êtes riche !!! Largement suffisant pour démmarer une nouvelle vie loin de tout problème et proche de toutes les babies dont vous rêvez!!!!", 0.01)
						fin_histoire()
					time.sleep(1)
			...
			décision = demander_choix("🔹 Quel est votre choix ?", choix_possibles[0])
			if int(décision) == 1 :
	...
	#Manifestation
		...
		reponse_2 = demander_choix("🔹 Quel est votre choix?", choix_possibles[2])
		if int(reponse_2) in [1, 2, 3, 4]:
			...
			duree_gav = 3        # durée de la garde à vue
			for jour in range(1, duree_gav + 1):
				time.sleep(0.75)
			...
			(reponse_2_1) = demander_choix("🔹 Quel est votre choix?", choix_possibles[0])
			...
			reponse_DEAL = demander_choix("🔹 Quel est votre choix?", choix_possibles[1])
			...
			if int(reponse_DEAL) == 1 :
				...
				choix_drogue = demander_choix("🔹 Quel est votre choix?", choix_possibles[0])
				...
				#Renoncer
					...
					money = sortie_GAV(money) #Scène sortie de la garde à vue
			...
			#Refuser le deal et dénoncer
				...
				time.sleep(1)
				choix_final = demander_choix("🔹 Quel est votre choix?", choix_possibles[0])
				...
				#Finir sous une dalle en béton
					...
					fin_histoire() #Fin du jeu
			money = le_cartel(money)  #Appel de la fonction pour rejoindre le cartel pour toute les situations restantes
